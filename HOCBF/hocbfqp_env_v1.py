...
class UnicycleHOCBFEnvRobotarium:
    ...
    def __init__(self,
        ...
        # Static obstacle
        self.static_center = np.array([0.5, 0.3])
        self.static_radius = 0.35

        # Defender capture radius
        self.capture_radius = 0.1

        self._num_obs = 2  # static circle + defender
    ...
    def reset(self):
        ...
        self.state_d = np.array([0.0, 0.0, -5*np.pi/6, 0.0], float)
        self.step_count = 0
    ...
    def _solve_qp_1d(u_nom, A_list, b_list):
        ...
        返回:
            u_safe: 解出的最优 u，如果无可行解，则返回 u_nom
            status_flag:
                0 -> QP 有解
                1 -> QP 无可行解（fallback 到 u_nom）
        """
        low = -np.inf
        high = np.inf
        tol = 1e-7  # 数值容差
        ...
        # 检查最终的区间是否有效 (下界不能大于上界)
        if low > high + tol:
            return float(u_nom), 1

        # 在有效区间 [low, high] 中，寻找距离 u_nom 最近的点
        # 解析解就是直接将 u_nom 截断(clip)在这个区间内
    ...
    def step(self, u_nom, k1_vec, k2_vec):
        """
        u_nom: nominal omega
        k1_vec: alpha_0j for each obstacle
        k2_vec: alpha_1j for each obstacle
        """
        ...
        # Defender motion (for dynamic obstacle center)
        ...
        info = {
            ...
            "min_safe_radius": float(min(self.static_radius, self.capture_radius)),
