...
class UnicycleHOCBFEnvRobotarium:
    ...
    def __init__(self,
        ...
        self.static_radii = np.array([0.3, 0.3, 0.3], dtype=np.float64)
        ...
        # 将防守者初始位置稍微靠后，给攻击者进通道的压力
        self.state_a = np.array([-1.2, -0.8, 2.5 * np.pi / 15.0, 0.0], dtype=np.float64)
        self.state_d = np.array([-0.6, -0.8, -np.pi / 6.0, 0.0], dtype=np.float64)
    ...
    def step(self, u_nom, k1_vec, k2_vec, alpha_c):
        ...
        # 遍历“障碍物”：i < 3 -> 静态圆, i == 3 -> 动态防守者捕捉圆
        for i in range(self._num_obs):
            if i < self._num_static_obs:
                # --- 静态圆障碍 ---
                c = self.static_centers[i]
                #避免实际控制机器人时，机器人会进入障碍物内部
                R = self.static_radii[i]+self.safe_margin  # 安全半径 = 几何半径 + 安全边距
                d = p_a - c
        ...
        # 安全半径集合（静态障碍 + 捕捉圆）
        radii_all = np.concatenate([self.static_radii, [self.capture_radius]])
